        ...
        if (!world.isRemote) {
            ...
            if (closestShrine != null && closestShrine.distance(player.position) < 400) {
                ...
                for (i in 0 until 20 - r.nextInt(4)) {
                    (world as WorldServer).spawnParticle(EnumParticleTypes.FIREWORKS_SPARK, player.posX + r.nextDouble()*1,
                            player.posY + r.nextDouble()*2, player.posZ + r.nextDouble()*1, 1,
                            dirVec.x + Math.random()*rand-rand/2, dirVec.y + Math.random()*rand-rand/2 + .1,
                            dirVec.z + Math.random()*rand-rand/2, 1.0)
                }
            }
        }
